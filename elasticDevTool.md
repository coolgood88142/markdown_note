...
- Compound queries(複合查詢)
  ...
  ```json
  //查詢user.id為kimchy，過濾production欄位有值的資料，
  POST _search
  {
    "query": {
      "bool" : {
...
- Span queries(跨越查詢)
  ...
  - Span not
    ...
    ```json
    //刪除與另一個跨度查詢重疊的匹配項，或者在另一個 SpanQuery之前（由參數控制pre）或之後（由參數控制）的x 個標記內的匹配項post。
    GET /_search
    {
      "query": {
        "span_not": {
  ...
  - Span or
    ...
    ```json
    //匹配其跨度子句的並集。跨度或查詢映射到 Lucene SpanOrQuery
    GET /_search
    {
      "query": {
        "span_or" : {
  ...
  - Span term
    ...
    ```json
    //匹配包含術語的跨度。span 術語查詢映射到 Lucene SpanTermQuery
    GET /_search
    {
      "query": {
        "span_term" : { "user.id" : "kimchy" }
...
- Specialized queries
  ...
  - Distance feature
    ...
    ```json
    //建立items索引值，name，一個keyword字段，production_date，一個date字段，location，一個geo_point字段
    PUT /items
    {
      "mappings": {
        "properties": {
